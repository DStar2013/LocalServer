...
var base = {
    ...
    getApiData: function(fileName, isBuffer) {
        var readableStream = !isBuffer ? fs.readFileSync(fileName, 'utf-8') : fs.readFileSync(fileName);
        return readableStream;
    },
    sendApiData: function(res, fileName, type) {
        var apiData;
        if (type == 'img') {
            apiData = this.getApiData(fileName, 'buffer');
        } else {
            apiData = JSON.parse(this.getApiData(fileName));
        }
        // logger.debug(apiData);
        this.apiOkOutput(res, apiData, type);
    }
};
module.exports = base;
